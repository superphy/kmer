...
def plot(data1, data2, label1, label2, title, max_x, o_type):
    ...
    for i, value in enumerate(labels):
        ...
        plt.plot(pos, np.zeros_like(pos)+spacing[i], 'o', color='blue', zorder=4, alpha = 0.4)
        plt.plot(neg, np.zeros_like(neg)+spacing[i], 'o', color='red', zorder=3, alpha = 0.4)
    blue_legend = plt.plot(np.NaN, np.NaN, 'o', color='blue', label=o_type + ' Sample')
    red_legend = plt.plot(np.NaN, np.NaN, 'o', color='red', label='Non-%s Sample'%o_type)
    plt.legend(fontsize=18)
    plt.yticks(spacing, [])
