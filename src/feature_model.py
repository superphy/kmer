...
if __name__ == "__main__":
	...
	for train,test in model_data:
		...
		if(num_feats!=0):
			...
			feat_index = np.zeros((1,2097152))
			for i in range(2097152):
				feat_index[0,i] = i
			feat_index = sk_obj.transform(feat_index)
			col_mask = np.zeros(2097152)
			cols = np.load('data/unfiltered/kmer_cols.npy')
			for i, col in enumerate(cols):
				if i in feat_index[0]:
					col_mask[i]=1
			col_mask = [i==1 for i in col_mask]
			#print(cols.shape, col_mask)
			cols = cols[col_mask]
			print(cols.shape)
			np.save('split'+str(split_counter)+'_'+str(num_feats)+'feats.npy', cols)
