...
def get_data(train, predict_for):
	...
	if(train in ('kmer, omnilog, Otype, Htype')):
		X = np.load('annotation/15mer_data/Host_kmer_matrix.npy', allow_pickle = True)
		Y = np.load('annotation/15mer_data/kmer_rows_Host2.npy', allow_pickle = True)
		print(X)
...
if __name__ == "__main__":
	...
	for train,test in model_data:
		...
		if(num_feats!=0):
			...
			if(imp_feats):
				if(train_string == 'omnilog'):
					...
					cols = np.load('annotation/15mer_data/Host_kmer_cols.npy', allow_pickle = True)
		...
		if(model_type == 'XGB'):
			...
			if(imp_feats):
				feat_save = 'annotation/15mer_data/imp/'+predict_for+'_'+str(num_feats)+'feats_'+model_type+'trainedOn'+train_string+'_testedOn'+test_string+'_fold'+str(split_counter)+'.npy'
				np.save(feat_save, np.vstack((cols.flatten(), [float(i) for i in sk_obj.scores_])))
