...
def create_temp_files():
    ...
    with open(directory + '/A1.fasta', 'w') as f:
        f.write('>\nAAACCCCAA')
    with open(directory + '/A2.fasta', 'w') as f:
        f.write('>\nAACCCCAA')
    with open(directory + '/B1.fasta', 'w') as f:
        f.write('>\nAACCAACC')
    files = [directory + '/A1.fasta', directory + '/A2.fasta', directory + '/B1.fasta']
...
class CountKmers(unittest.TestCase):
    def setUp(self):
        ...
        count_kmers(self.files, self.db, k=2)
        ...
        with env.begin(write=False, db=A1) as txn:
            with txn.cursor() as cursor:
                for key, val in cursor:
                    if key != 'complete_results'.encode():
                        count2 += 1
                    key = key.decode()
                    val = val.decode()
                    if key == 'AA' and val == '3':
...
#         self.dir, self.db, self.files = create_temp_files()
#         count_kmers(self.files, self.db, k=2)
#         self.new_file = self.dir + '/B2.fasta'
...
class GetKmerNames(unittest.TestCase):
    def setUp(self):
        ...
        with open(self.fasta, 'w') as f:
            f.write('>l1\nATAT\n>l2\nCGCG\n>l3\nAAAA\n>l4\nAGGA\n>l5\nCGCG')
        count_kmers([self.fasta], self.db, k=4)
    ...
    def test_names(self):
        ...
        if np.array_equal(correct_names, self.names):
            ...
            val = False
        self.assertTrue(val, msg=self.names)
