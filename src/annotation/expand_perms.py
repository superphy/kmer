...
if __name__ == "__main__":
    ...
    top_feats = []
    all_feats = np.load('data/features/Host_110feats_XGBtrainedOn'+dataset+'_testedOn'+dataset+'_fold1.npy', allow_pickle = True)

    if(isinstance(all_feats[0,0],bytes)):
        all_feats = [[i.decode('utf-8') for i in all_feats[0]],[float(i.decode('utf-8')) for i in all_feats[1]]]
        np.save('data/features/Host_110feats_XGBtrainedOn'+dataset+'_testedOn'+dataset+'_fold1.npy', all_feats)

    for row in all_feats:
        for col in row:
            assert(not isinstance(col,bytes))
    ...
    if not os.path.exists(os.path.abspath(os.path.curdir)+"/annotation/15mer_data/"):
        os.makedirs(os.path.abspath(os.path.curdir)+"/annotation/15mer_data/")

    np.save("annotation/15mer_data/Host_110feats_{}_15mers.npy".format(dataset), OxF_mer_matrix)
    np.save("annotation/15mer_data/Host_110feats_{}_15mers_parent.npy".format(dataset), OxB_labels)
