...
if __name__ =="__main__":
    ...
    kmer_matrix = np.load("annotation/15mer_data/Host_kmer_matrix.npy", allow_pickle = True)
    y_train = np.load("annotation/15mer_data/kmer_rows_Host2.npy", allow_pickle = True)
    kmer_cols = np.load("annotation/15mer_data/Host_kmer_cols.npy", allow_pickle = True)
    '''
    ...
    kmer_rows_mic = [mic_df[Host][i] for i in kmer_rows_genomes]
    num_starting_mic = len(kmer_rows_mic)

    # keep only rows with valid MIC labels
    assert(type(kmer_rows_mic[0])==type(mic_class_dict[Host][0]))
    row_mask = [i in mic_class_dict[Host] for i in kmer_rows_mic]
    ...
    if(num_removed>0):
        print("Removed {} invalid rows from {} kmer_matrix".format(num_removed,Host))

    # encode mic labels from 1,2,4,8 to 0,1,2,3
    le = preprocessing.LabelEncoder()
    le.classes_ = mic_class_dict[Host]
    ...
    for i, col in enumerate(kmer_matrix.T):
        ...
        for class_num in range(len(mic_class_dict[Host])):
    ...
    feat_save = 'annotation/15mer_data/{}_{}_feature_ranks.npy'.format(Host,dataset)
