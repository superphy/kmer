...
def acc(input_file, model=None, data=None, prediction=None, extra_col=None, extra_val=None):
    ...
    if model is not None:
        df = df.loc[df['Model Type'] == model]
    if data is not None:
        df = df.loc[df['Data Type'] == data]
...
def main():
    ...
    multiclass_file = snakemake.input[2]
    otype_file = snakemake.input[3]
    serotype_file = snakemake.input[4]
    validation_file = snakemake.input[5]
    ...
    data = acc(validation_file, data='kmer', model='Neural Network',
               extra_col='Dataset', extra_val='split')
    output['ValidationNNSplit'] = data[0]

    data = acc(validation_file, data='kmer', model='Neural Network',
               extra_col='Dataset', extra_val='mixed')
