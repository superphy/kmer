...
class AugmentData(unittest.TestCase):
    def setUp(self):
        ...
        self.features = 6
        x_train = np.random.randint(10, size=(self.original_samples,
                                    self.features))
        ...
        for x in range(len(values)):
            self.correct_y_train.extend([values[x]] * (counts[x] +
                                        self.new_samples))

    def test_dimensions(self):
        tot_samples = self.original_samples + (self.new_samples * self.classes)
        correct_dims = [(tot_samples, self.data[0].shape[1]), (tot_samples, ),
                        self.data[2].shape, self.data[3].shape]
